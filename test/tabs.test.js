...
/*
 * 24 published pages emit a `tabs` block and this repo had no implementation, so
 * /blocks/tabs/tabs.js and /blocks/tabs/tabs.css both answered 404 on every one of those
 * page loads, and the five panels rendered stacked and all visible.
 *
 * Read on /en-gb/general-terms-and-conditions: five rows, each with two children, the first
 * the tab label ("General terms and conditions", "Legal mentions", "Privacy policy",
 * "Security", "Cookies") and the second the panel body. general-terms-and-conditions in ten
 * locales, our-network and the miles calculator make up the 24.
 *
 * The block is adobe/aem-block-collection's, which expects exactly that structure: each row's
 * first element child is the label and the row becomes the panel. The same move decision 5
 * made for the FAQ: reuse rather than write.
 */
...
describe('the tabs block exists and keeps the collection contract', () => {
  ...
  // One selector per line here, so a line ending in `{` is a selector unless it opens an
  // at-rule. A regex over the whole file matches declaration bodies instead, because @media
  // nests, which is how my first version of this test failed on CSS that was already scoped.
  it('scopes every selector to the block', () => {
    const unscoped = css.split('\n')
      .map((l) => l.trim())
      .filter((l) => l.endsWith('{') && !l.startsWith('@'))
      .map((l) => l.slice(0, -1).trim())
      .filter((l) => !/(^|\s|,)\.tabs\b/.test(l));
